	...
	<div class="card">
		...
		<div class="card-body">
			<ul class="nav nav-pills nav-secondary nav-pills-no-bd" id="pills-tab-without-border" role="tablist">
				...
				<li class="nav-item">
					<a class="nav-link" id="pills-email-tab-nobd" data-toggle="pill" href="#pills-email-nobd" role="tab" aria-controls="pills-email-nobd" aria-selected="false">Ganti Email</a>
				</li>
			</ul>
			<div class="tab-content mt-2 mb-3" id="pills-without-border-tabContent">
				<div class="tab-pane fade show" id="pills-home-nobd" role="tabpanel" aria-labelledby="pills-home-tab-nobd">
					...
					if (isset($_POST['changePassword'])) {
						...
						$query = mysqli_query($con, "SELECT password FROM tb_pemilik WHERE id_pemilik='$data[id_pemilik]'");
						...
						if (password_verify($passLama, $passDatabase)) {
							...
							$set = mysqli_query($con, "UPDATE tb_pemilik SET password='$newPass' WHERE id_pemilik='$data[id_pemilik]' ");
            ...
            Toast.fire({
				...
				<div class="tab-pane fade" id="pills-profile-nobd" role="tabpanel" aria-labelledby="pills-profile-tab-nobd">
					<form action="" method="post" enctype="multipart/form-data">
						<div class="form-group">
							...
							<input type="hidden" name="id" value="<?= $data['id_pemilik'] ?>">
						</div>
						<div class="form-group">
					...
					if (isset($_POST['updateProfile'])) {
						...
						if (!empty($gambar)) {
							...
							$ganti = mysqli_query($con, "UPDATE tb_pemilik SET foto='$gambar' WHERE id_pemilik='$_POST[id]' ");
							if ($ganti) {
								echo "
													<script>
                        const Toast = Swal.mixin({
				...
				<div class="tab-pane fade" id="pills-email-nobd" role="tabpanel" aria-labelledby="pills-email-tab-nobd">
					<form action="" method="post">
						<div class="form-group">
							<input name="emailBaru" type="email" class="form-control" placeholder="Email Baru">
						</div>
						<div class="form-group">
							<button name="changeEmail" type="submit" class="btn btn-info btn-block">Ganti Email</button>
						</div>
					</form>
					<?php
					if (isset($_POST['changeEmail'])) {
						...
						// Lakukan validasi dan proses ganti email di sini
						// ...

						// Contoh validasi sederhana:
						if (filter_var($emailBaru, FILTER_VALIDATE_EMAIL)) {
							// Proses update email ke dalam database
							$updateEmail = mysqli_query($con, "UPDATE tb_pemilik SET email='$emailBaru' WHERE id_pemilik='$data[id_pemilik]' ");
							...
							if ($updateEmail) {
								...
								Toast.fire({
									icon: 'success',
									title: 'Berhasil! Email berhasil diperbarui'
								});
								setTimeout(function () { 
									window.location.replace('?page=akun');
								}, 3000);   
								...
								Toast.fire({
									icon: 'error',
									title: 'Gagal! Terjadi kesalahan saat mengubah email'
								});
								</script>";
							}
						} else {
